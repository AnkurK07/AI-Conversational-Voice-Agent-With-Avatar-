...
INSTRUCTIONS = """You are a helpful voice assistant. Have natural conversations with users and provide helpful responses to their questions and requests. Keep your responses conversational and engaging."""


class ConversationVoiceAssistant(Agent):
    def __init__(self) -> None:
        super().__init__(
            instructions=INSTRUCTIONS,
            # Remove tools parameter - no tools needed for conversation
        )
    
    async def entrypoint(self, ctx: agents.JobContext):
        """Agent entrypoint for conversation-only assistant"""
        await ctx.connect()
        ...
        session = AgentSession(
            ...
            tts=deepgram.TTS(
                model="aura-asteria-en",  # High quality voice (Visit Deepgram Doccumentation)
            ...
            # Voice Activity Detection Selerio, SET UP Selerio API Firts
            vad=silero.VAD.load(),
        )
...
async def entrypoint(ctx: agents.JobContext):
    ...
    assistant = ConversationVoiceAssistant()
